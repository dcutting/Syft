...
public enum MatchResult: MatchResultLike, Equatable, Printable {
    ...
    case Leaf([String: MatchResultLike], remainder: String)
    ...
    public var description: String {
        ...
        case let .Match(match: match, index: index, remainder: remainder):
            return "\"\(match)\"@\(index)"
        case let .Leaf(hash, remainder: remainder):
            var str = ""
...
public func ==(lhs: MatchResult, rhs: MatchResult) -> Bool {
    ...
    case let (MatchResult.Leaf(lhsHash, remainder: lhsRemainder), MatchResult.Leaf(rhsHash, remainder: rhsRemainder)):
